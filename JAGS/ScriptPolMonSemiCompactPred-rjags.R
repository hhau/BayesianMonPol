##
## R script to drive the files PolMonSemiCompactEvenPred.bug and
## PolMonSemiCompactOddPred.bug, i.e. to fit polynomials that are
## constrained to be montone over a semi compact interval of the form
## [a, infty)
##
...
## monotone over the interval [0, infty] to these data
##
K <- 2
q <- 2*K         ## choose whether the polynomial is supposed to be even
## q <- 2*K + 1  ## or odd by commenting out one line

alpha <- 1  ## we want the polynomial to be monotone increasing on [0, infty)
a <- 0
...
##
if( q%%2 == 0 ){
  m <- jags.model("PolMonSemiCompactEvenPred.bug",
  ...
  m <- jags.model("PolMonSemiCompactOddPred.bug",
                  data=list(N=N, x=x, y=y, q=q, K=K, 
                      alpha=alpha, a=a,
                      xnew=xnew, Nnew=Nnew),
                  inits=inits,
